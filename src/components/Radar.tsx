...
export default function Radar({
  ...
  useEffect(() => {
    ...
    function update(time: number) {
      ...
      if (enableMouseInteraction) {
        currentMouse[0] = (currentMouse[0] ?? 0.5) + 0.05 * ((targetMouse[0] ?? 0.5) - (currentMouse[0] ?? 0.5));
        currentMouse[1] = (currentMouse[1] ?? 0.5) + 0.05 * ((targetMouse[1] ?? 0.5) - (currentMouse[1] ?? 0.5));
